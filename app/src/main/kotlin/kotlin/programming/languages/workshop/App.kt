...
class Search : CliktCommand() {
    //options
    val pattern: String by argument(help = "substring to search for")
    val path: String by argument(help = "file system path")

    override fun run() {
        val searcher = Searcher()
        searcher.recursiveFileSearch(path, searcher.preprocess(pattern))
    }
}
